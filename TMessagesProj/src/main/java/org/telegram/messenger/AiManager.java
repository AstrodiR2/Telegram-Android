...
public class AiManager {
    ...
    public static void ask(Context context, long dialogId, String question, String senderName, String senderUsername, long senderId, String groupHistory, AiCallback callback) {
        ...
        String systemPrompt = getRolePrompt(ROLE_CHAT_AGENT) + " When using web search results, never mention, list, or cite your sources, URLs, or links in the response. Just answer using the information naturally, as if you already knew it." +
            ...
            " STICKER: You have a sticker pack. Use [STICKER:emoji] to send a sticker instead of or alongside text when it fits the vibe. Available stickers: 🖼=don't care/похуй, 😳=refusal/wtf, 🦁=messed up/проебался, 🤤=zoned out/залип, 😎=shut up/завали, ✋=stop/отказ, 🌬=bullshit/воздух. Don't always use stickers — only when it genuinely fits the vibe. Can respond with text only, sticker only, or both." +
            sysExtra.toString();
        new Thread(() -> {
            try {
